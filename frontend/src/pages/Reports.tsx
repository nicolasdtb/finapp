...
import { Transaction, Category, Account, Tag } from "../services/api.js";
...
export const Reports: React.FC<ReportsProps> = ({ transactions, categories, accounts, tags }) => {
  const [cycleOffset, setCycleOffset] = useState<number>(0);
  const [reportType, setReportType] = useState<"expenses" | "income">("expenses");
  const [viewMode, setViewMode] = useState<"category" | "tags">("category");
  ...
  // Agrupamento por Tag (inclui tags de transações E tags de itens individuais)
  // Para tags de item: o valor contabilizado é apenas o do item (not the full tx)
  const tagStats = useMemo(() => {
    const targetTypeId = reportType === "expenses" ? 2 : 1;
    const targetTransactions = cycleTransactions.filter(t => t.typeId === targetTypeId);
    const targetTotal = targetTypeId === 2 ? totalExpenses : totalIncome;

    const map = new Map<number, { tag: Tag; amount: number; count: number }>();

    targetTransactions.forEach(t => {
      const txAmount = parseFloat(t.amount || "0");

      // 1. Tags da transação principal (valor cheio da transação)
      (t.tagIds || []).forEach(tagId => {
        ...
        if (existing) {
          ...
          map.set(tagId, { tag, amount: txAmount, count: 1 });
        }
      });

      // 2. Tags dos itens individuais (valor do item específico)
      if (t.items && t.items.length > 0) {
        t.items.forEach(item => {
          ...
          (item.tagIds || []).forEach(tagId => {
            const tag = tags.find(tg => tg.id === tagId);
            if (!tag) return;
            // Só adiciona tag de item se ela NÃO foi contabilizada como tag da transação inteira
            const alreadyOnTx = (t.tagIds || []).includes(tagId);
            if (alreadyOnTx) return;
            const existing = map.get(tagId);
            if (existing) {
              existing.amount += itemAmount;
              existing.count += 1;
            } else {
              map.set(tagId, { tag, amount: itemAmount, count: 1 });
            }
          });
        });
      }
  ...
  }, [cycleTransactions, reportType, tags, totalExpenses, totalIncome]);
  ...
  return (
    <div className="pb-28 pt-4 px-4 max-w-md mx-auto sm:max-w-2xl">
      ...
      <div className="grid grid-cols-2 gap-2 bg-slate-950 p-1.5 rounded-2xl mb-3 border border-slate-800">
        <button
      ...
      <div className="grid grid-cols-2 gap-2 bg-slate-950 p-1.5 rounded-2xl mb-5 border border-slate-800">
        <button
          ...
          className={`py-2 text-xs font-bold rounded-xl transition flex items-center justify-center gap-1.5 ${
            viewMode === "category" ? "bg-blue-600 text-white shadow-md" : "text-slate-400 hover:text-slate-200"
          }`}
        >
        ...
        <button
          ...
          className={`py-2 text-xs font-bold rounded-xl transition flex items-center justify-center gap-1.5 ${
            viewMode === "tags" ? "bg-violet-600 text-white shadow-md" : "text-slate-400 hover:text-slate-200"
          }`}
        >
                  ...
                  <div key={idx} className="space-y-1.5">
                    <div className="flex justify-between items-center text-xs">
                      <div className="flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: item.category.color || "#3B82F6" }} />
                    ...
                    <div className="w-full h-1.5 bg-slate-800/80 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${item.percentage}%`, backgroundColor: item.category.color || "#3B82F6" }}
                      />
                    </div>
                  </div>
                ))
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-xl">
              ...
              <div className="space-y-2.5">
                {topExpenses.map((tx, idx) => (
                  <div key={tx.id} className="p-3 bg-slate-950/60 border border-slate-800/80 rounded-2xl flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-xl">
          <div className="flex justify-between items-center mb-3">
            <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center gap-1.5">
              ...
              {reportType === "expenses" ? "Gastos" : "Receitas"} por Tag
            </h3>
            <span className="text-xs text-slate-400 font-medium">
          ...
          {tagStats.length === 0 ? (
            <div className="py-8 text-center">
              <TagIcon className="w-8 h-8 text-slate-700 mx-auto mb-2" />
              <p className="text-xs text-slate-500">Nenhuma tag vinculada a transações neste período.</p>
              <p className="text-[10px] text-slate-600 mt-1">Edite seus lançamentos e adicione tags para ver o relatório aqui.</p>
            </div>
          ) : (
            <>
              {/* Barra multicolorida por tags */}
              <div className="w-full h-3 bg-slate-800 rounded-full overflow-hidden flex mb-5">
                {tagStats.map((item, idx) => (
                  <div
                    key={idx}
                    style={{ width: `${item.percentage}%`, backgroundColor: item.tag.color }}
                    title={`${item.tag.name}: ${item.percentage.toFixed(1)}%`}
                    className="h-full transition-all hover:opacity-80"
                  />
                ))}
              </div>
                    ...
                    <div className="flex justify-between items-center text-xs">
                      <div className="flex items-center gap-2">
                        <span
                          ...
                          style={{ backgroundColor: item.tag.color + "20", color: item.tag.color, borderColor: item.tag.color + "50" }}
                        >
